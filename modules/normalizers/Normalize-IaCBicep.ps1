...
function Normalize-IaCBicep {
    ...
    foreach ($finding in $ToolResult.Findings) {
        ...
        # For IaC validation, ResourceId is a file path within the repo.
        ...
        if ($rawId) {
            try {
                ...
                if (-not $slug) { $slug = 'local' }
                $canonicalId = "iac.local/bicep-iac/$slug"
            }
        }
        ...
        if (-not $canonicalId) {
            $canonicalId = "iac.local/bicep-iac/$findingId"
        }
        ...
        $row = New-FindingRow -Id $findingId `
            -Source 'bicep-iac' -EntityId $canonicalId -EntityType 'Repository' `
