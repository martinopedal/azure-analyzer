...
- **PR #1012 rebase (2026-05-09):** Rebased `copilot/create-easm-plan-and-reports` from 76f317b (PR #1011) onto bc46e70 (PR #1013, current main). CHANGELOG.md had dual conflict: (1) release-please added "Scheduled Scan OIDC preflight" entry at line 10, (2) PR's commit 638bec7 added "EASM PR review feedback" entry at same location, (3) subsequent commit c22c58e appended more fixes to the same entry. Resolution: kept release-please block on top (chronological), then full EASM entry below. All 9 PR commits rebased cleanly. Pushed with `git push --force-with-lease origin pr-1012-rebase:copilot/create-easm-plan-and-reports`. Post-rebase state: `mergeable: MERGEABLE`, `mergeStateStatus: BLOCKED` (checks pending). No code changes — only CHANGELOG merge strategy.
- **Auto-bumper catalog regen fix (2026-05-08, #1020):** Update-ToolPins.ps1 was updating tools/tool-manifest.json but not regenerating the derived catalog docs (docs/reference/tool-catalog.md + tool-catalog-contributor.md), causing all 22 open auto-bump PRs (#985-#1018) to fail CI on Generate-ToolCatalog.Tests.ps1 ("CheckOnly succeeds" + "committed catalogs match"). Root cause: missing catalog regen step between manifest write and git add. Fix: invoke `pwsh -File scripts/Generate-ToolCatalog.ps1` after the manifest update and stage both catalog files alongside the manifest in the same commit, making the update atomic. Added error handling to fail loudly if the generator exits nonzero. The PR body template now references umbrella issue #1019 to satisfy closes-link-required.yml. This is a "manifest changes must regen sidecar artifacts" pattern that applies to any derived/generated artifact tied to a source-of-truth file.
...
### Issue #1062 refactor: PR review-gate external calls (2026-05-13)

Wrapped all `gh api` and `gh pr comment` calls in `modules/shared/Invoke-PRReviewGate.ps1` with `Invoke-WithRetry` for transient failure resilience (429/503/504/throttle/timeout). Pattern matches existing wrappers (`Invoke-AdoConsumption.ps1`): dot-source `Retry.ps1` with inline test-compat fallback stub, wrap call sites in `Invoke-WithRetry -ScriptBlock { ... } -MaxAttempts 3`. Added 2 integration tests verifying transient 503 retry (succeeds on 2nd attempt) and non-transient 401 immediate failure. All 16 PRReviewGate tests pass. PR #1070.
- **Bulk tool-catalog regen refresh (2026-04-22, #985-#1018):** All 22 open auto-bump PRs had stale `docs/reference/tool-catalog*.md` files relative to their manifest pins. Scripted refresh: checkout each branch → `pwsh -File scripts/Generate-ToolCatalog.ps1` → commit (github-actions bot, Copilot trailer) → force-push --force-with-lease. All 22 committed cleanly. Closes #1019 link backfill added to 5 PRs (spot check showed rest already had it). CI sample: 4 PRs monitored; 2 already passing, 2 pending (no failures). This demonstrates the **atomic-update pattern**: manifest changes MUST immediately regenerate derived catalogs in the same commit to avoid the stale-doc trap. Pending CI validation before merge batch.
...
## 2026-05-12 - PR #1044 merged: triage filter for bot-managed bump branches (closes #1040)

Triage of issue #1040 (auto-generated "untriaged CI failures in last 24h", 49 entries):

- 42 of 49 (~95%) were stale failed runs from `chore/bump-*` branches generated by `tools/Update-ToolPins.ps1`. Each weekly bump opens its own PR; superseded bumps left old failures lingering even though the work is tracked under umbrella #1019.
- 7 of 49 were unrelated and none blocked main (recent main CI green).

Verdict: pure policy noise. Patched at the source.

### Patches

1. `.github/workflows/ci-health-digest.yml` - digest now drops runs whose `head_branch` matches `^(chore/bump-|release-please--)` before assembling the issue body. Both bot families have their own remediation loops (umbrella #1019 for bumps, release-please for releases) and surface real failures via per-PR required checks.
2. `.github/workflows/closes-link-required.yml` - early-skip for `chore/bump-*` head refs mirroring the existing `release-please--*` skip block. Defense in depth against body-edit timing races that could drop the `Closes #1019` marker the bumper currently writes.

### Tests

- `tests/workflows/CiHealthDigest.Tests.ps1` - new `It` block requires the digest to filter the bot branch families via a `$botBranchPattern` guard.
- `tests/workflows/ClosesLinkRequired.Tests.ps1` - new `It` block requires the workflow to skip `chore/bump-*` with an umbrella `#1019` reference.
- 508/508 workflow tests green locally.

### Learnings

- **Daily triage digest ignored bot branch families.** The skip-list (lines 97-101 of ci-health-digest.yml) only excluded the digest's own workflow names. It did not exclude bot-generated branch families, so when the bumper opens 12 PRs in a window each with 3 noisy required checks (Closes Link Required, Docs Check, CI), a single digest run can surface 36+ "untriaged" entries that are all stale by the time anyone reads them.
- **Real product breakage on `main` is preserved.** The exclusion is keyed on `head_branch` matching a bot prefix; main pushes use a bare `main` branch name, so this filter does not hide genuine main breakage.
- **Defense in depth at the source matters.** Even though the bumper PR body always includes `Closes #1019`, a manual rebase or `gh pr edit --body` race can drop the marker. The `closes-link-required.yml` skip ensures the noise cannot reappear if that template ever drifts.
