...
**Total enabled:** 38. **Disabled / opt-in:** 14.
...
| `conditional-access-graph` | Conditional Access Policy Graph | tenant | graph | NIST 800-53, CIS Controls v8, SOC2 | Conditional Access Policy Graph | - |
...
| `azurehound` | AzureHound (SpecterOps attack-path collector) | tenant | graph | NIST 800-53 | AzureHound (SpecterOps attack-path collector) |
| `bloodhound-ce` | BloodHound CE (Neo4j ingest helper) | tenant | graph | NIST 800-53 | BloodHound CE (Neo4j ingest helper) |
...
| `entra-permissions-mgmt` | Microsoft Entra Permissions Management (RBAC right-sizing) | tenant | graph | NIST 800-53, SOC2 | Microsoft Entra Permissions Management (RBAC right-sizing) |
| `forest-druid` | Semperis Forest Druid (commercial, gated) | tenant | graph | NIST 800-53 | Semperis Forest Druid (commercial, gated) |
| `graphrunner` | GraphRunner (post-compromise Graph enumeration, passive) | tenant | graph | NIST 800-53 | GraphRunner (post-compromise Graph enumeration, passive) |
| `httpx` | httpx (ProjectDiscovery host probe) | tenant | easm | NIST 800-53 | httpx (ProjectDiscovery host probe) |
| `pim-graph` | PIM Eligibility / Activation Graph | tenant | graph | NIST 800-53, SOC2, PCI-DSS | PIM Eligibility / Activation Graph |
| `roadrecon` | ROADrecon (Dirk-jan Mollema OSS Entra graph) | tenant | graph | NIST 800-53 | ROADrecon (Dirk-jan Mollema OSS Entra graph) |
