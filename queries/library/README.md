...
- Files in per-tool subfolders of `queries/` (e.g. `queries/alz/alz_additional_queries.json`, `queries/finops/finops-*.json`) are **read by a wrapper** - the orchestrator depends on them.
...
3. Do **not** put files here if a wrapper actually reads them - those belong in the wrapper's per-tool subfolder under `queries/` (e.g. `queries/alz/`, `queries/finops/`).
4. The orphan-query audit (see `.squad/decisions.md` → ALZ Queries SoT section) treats anything in a runtime subfolder of `queries/` not referenced by a wrapper as a candidate for either wiring up, deletion, or moving here. If in doubt, default to wiring up.
