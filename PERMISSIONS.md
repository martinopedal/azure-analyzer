...
Markdown Check lychee retry hardening (cache reset between attempts + `GITHUB_TOKEN` for GitHub URL checks) only changes CI resiliency and introduces no new runtime scanner permissions or scopes.
...
Log Analytics sink error-model hardening (`New-FindingError` migration with mandatory `Remediation` + credential-sanitized `Details`, plus sink ratchet coverage) changes error formatting only and introduces no new Azure RBAC, Graph scopes, GitHub scopes, or ADO scopes.
Release automation (release-please, GitHub Releases, and PSGallery publication) introduces repository automation and package feed credentials (`PSGALLERY_API_KEY`) only; it does not require any additional Azure, Microsoft Graph, GitHub repository-content write scopes for runtime scanners, or Azure DevOps read scope changes. Release-please uses a GitHub App token (via `actions/create-github-app-token`) instead of `GITHUB_TOKEN` so that PRs it creates trigger downstream CI workflows. The App requires `contents: write` and `pull-requests: write` permissions, installed on this repository only. App credentials are stored as repo secrets `RELEASE_APP_ID` and `RELEASE_APP_PRIVATE_KEY`.
Bot-gate unification (#938): the same GitHub App token (`RELEASE_APP_ID` / `RELEASE_APP_PRIVATE_KEY`) is now used by `tool-auto-update.yml` (PR creation) and `pr-auto-rebase.yml` (force-push to PR branches) so that bot-authored activity triggers downstream CI workflows and bypasses the first-time-contributor approval gate. No new App permissions are required; the existing `contents: write` and `pull-requests: write` installation scopes cover both workflows.
Cross-App review-thread resolution (`pr-auto-resolve-threads.yml`) uses a separate GitHub App, `azure-analyzer-thread-resolver`, minted in-job via `actions/create-github-app-token` (secrets `RESOLVE_THREADS_APP_ID` and `RESOLVE_THREADS_PRIVATE_KEY`). The default `GITHUB_TOKEN` cannot resolve review threads it did not author (GraphQL `resolveReviewThread` returns FORBIDDEN on bot-vs-bot resolution, e.g. when an agent push needs to resolve a Copilot Code Review thread); a first-class App actor bypasses that block. Required App installation scope is `pull_requests: write` on this repository only. Without that scope every cross-App resolve call fails and Copilot threads have to be resolved by hand on every agent PR. No Azure, Microsoft Graph, or Azure DevOps scopes are added.
...
Azure DevOps wrapper authentication (`ado-pipelines`, `ado-connections`, `ado-repos-secrets`, `ado-consumption`, `ado-pipeline-correlator`) accepts either a classic PAT or a Microsoft Entra access token issued for the Azure DevOps resource `499b84ac-1321-427f-aa17-267ca6975798`. Both travel in the same HTTP Basic password slot via the shared `Resolve-AdoPat` helper, so this is a documentation clarification only: no new Azure DevOps scopes, and no new Azure, Microsoft Graph, or GitHub permissions. The Entra token carries the same read scopes the identity already holds and is typically valid for ~1 hour, versus a long-lived PAT. See [docs/consumer/ado-auth.md](docs/consumer/ado-auth.md).
