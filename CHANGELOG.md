...
### Added
- **Scorecard enterprise-host support (`-GitHubHost`) for GHEC-DR/GHES custom domains**: `Invoke-AzureAnalyzer.ps1` now defaults `-GitHubHost` to `github.com` and passes it to the Scorecard wrapper; `Invoke-Scorecard.ps1` sets `GH_HOST` for each run (with restore in `finally`) so enterprise domains such as `company.ghe.com` and `ghes.internal.corp` work without manual environment-variable setup.
