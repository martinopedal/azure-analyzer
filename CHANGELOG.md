...
- feat(terraform-iac): upgraded `modules/iac/IaCAdapters.ps1` + `modules/normalizers/Normalize-IaCTerraform.ps1` to Schema 2.2 ETL end to end. Wrapper now emits per-rule `Pillar`, `Frameworks` (`Azure WAF`, `CIS Azure`, `Azure Security Benchmark`, `NIST 800-53` where mapped), `DeepLinkUrl`, `RemediationSnippets`, `EvidenceUris` (file plus line anchors, GitHub blob URL when remote), `BaselineTags` (`terraform:rule:*`, provider, tool), `EntityRefs`, and `ToolVersion` from terraform/trivy/tfsec/checkov probes. Normalizer maps severity (`CRITICAL/HIGH/MEDIUM/LOW/UNKNOWN` to all five schema levels), preserves Schema 2.2 fields, and emits `IaCFile` entity IDs (`iac:terraform:{relative-path}#{resource_address}`) for stable EntityStore dedup. Added fixture-backed normalizer coverage and schema tests for `IaCFile` entity type (closes #376).
